...
export default function ClientesPage() {
  ...
  const handleEditClient = async () => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:3000/api/modules/agenda/clients/${selectedCliente.id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          ...(token ? { Authorization: `Bearer ${token}` } : {}),
              ...
              <div className="flex flex-col md:flex-row md:items-center gap-4">
                <div className="relative flex-1 w-full">
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2 border rounded-lg focus:ring-2 focus:ring-bivoo-purple focus:outline-none text-sm sm:text-base"
                  />
                ...
                <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
                  ...
                  <select
                    className="border rounded-lg px-3 py-2 text-sm sm:text-base w-full sm:w-auto focus:ring-2 focus:ring-bivoo-purple focus:outline-none"
                  >
                    <option>Todos</option>
