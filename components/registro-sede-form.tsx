...
export default function RegistroSedeForm({ onSuccess, onCancel }: { onSuccess?: (newSede?: NewSede) => void; onCancel?: () => void }) {
      ...
      interval = setInterval(() => {
        ...
        if (step >= loaderMessages.length) {
          ...
          setTimeout(() => {
            ...
            const createdName = formData.companyName;
            ...
            if (registrationSucceeded) {
              // Informar al padre y pasar datos básicos de la nueva sede
              onSuccess?.({ nombre: createdName || '', direccion: '', ciudad: '', telefono: '', principal: false });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // en caso de éxito, el efecto que maneja `showLoader` notificará al padre y cerrará el modal
  };
    ...
    <div className="w-full max-w-3xl mx-auto bg-white rounded-2xl shadow-2xl p-6 sm:p-10 border border-gray-200">
      <form onSubmit={handleSubmit}>
        ...
        <div className="mt-6 flex items-center justify-end gap-3">
          <button type="button" onClick={() => onCancel ? onCancel() : router.push('/configuracion/empresa')} className="inline-flex items-center px-4 py-2 text-sm text-gray-600 bg-gray-50 border rounded hover:bg-gray-100">Cancelar</button>
          <button type="submit" onClick={handleNext} className={`inline-flex items-center gap-2 px-4 py-2 rounded font-semibold text-white shadow ${loading ? 'opacity-60 cursor-not-allowed' : 'bg-gradient-to-r from-[#A259FF] to-[#7E22CE]'}`} disabled={loading}>{loading ? (<span className="animate-spin mr-2 h-4 w-4 border-b-2 border-white rounded-full"></span>) : 'Registrar y Completar'}</button>
        </div>
      </form>
